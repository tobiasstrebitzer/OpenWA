...
- **MCP server (opt-in, off by default):** AI agents (Claude, Cursor, etc.) can drive WhatsApp as first-class tools over the [Model Context Protocol](https://modelcontextprotocol.io). Set `MCP_ENABLED=true` to mount a stateless Streamable-HTTP transport at `POST /mcp` on the existing server (same port, no extra process). It is **additive**: existing controller routes opt in with a single `@Mcp()` decorator and keep serving REST unchanged; ~126 routes are exposed as tools. Every tool call goes through the same `ApiKeyGuard` + `@RequireRole` + per-key session scoping as REST (unit + e2e covered), so a key scoped to one session cannot act on another. Built directly on the official `@modelcontextprotocol/sdk`; when `MCP_ENABLED` is unset, none of it loads.
