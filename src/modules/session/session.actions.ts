...
export class SessionActions {
  ...
  @Action({
    description: 'Get active chats for a session',
    input: IdInput,
    kind: 'query',
    method: 'GET',
    path: 'sessions/:id/chats',
  })
  getChats(input: z.infer<typeof IdInput>) {
    return this.sessionService.getChats(input.id);
  }

  @RequireRole(ApiKeyRole.OPERATOR)
  @Action({
    description: 'Mark a chat as read/seen',
    input: MarkChatReadInput,
    method: 'POST',
    path: 'sessions/:id/chats/read',
  })
  async markChatRead(input: z.infer<typeof MarkChatReadInput>): Promise<{ success: boolean }> {
    const success = await this.sessionService.sendSeen(input.id, input.chatId);
    return { success };
  }
