...
// Mirrors `SendTemplateMessageDto`: render a stored template and send it as text.
// Exactly one of templateId / templateName must be supplied (the service enforces it).
const SendTemplateInput = z.object({
  ...
  templateId: z
    ...
    .min(1)
    .optional()
    .describe('Template ID to render (provide either templateId or templateName)'),
  templateName: z
    .string()
    .min(1)
    .optional()
    .describe('Template name to render (provide either templateId or templateName)'),
  vars: z.record(z.string(), z.string()).optional().describe('Variables substituted into {{placeholder}} tokens'),
});

const GetChatHistoryInput = z.object({
  sessionId: z.string().describe('Session ID'),
  chatId: z.string().describe('Chat ID (e.g. 1234567890@c.us or groupId@g.us)'),
  limit: z.coerce
    .number()
    .int()
    .positive()
    .optional()
    .describe('Max messages to return (default 50, clamped to [1,100])'),
  // Boolean-or-string union (no .transform — transforms can't be represented in the
  // JSON Schema that Swagger/MCP generate). The handler does the truthy conversion so
  // it matches the old controller: only `true` / "true" / "1" enable media download.
  includeMedia: z
    .union([z.boolean(), z.string()])
    .optional()
    .describe('Download media (base64) for messages that have it. Slower; default false.'),
});
...
export class MessageActions {
  ...
  @RequireRole(ApiKeyRole.OPERATOR)
  @Action({
    description: 'Render a stored text template and send it as a text message',
    input: SendTemplateInput,
    method: 'POST',
    path: 'sessions/:sessionId/messages/send-template',
  })
  sendTemplate(input: z.infer<typeof SendTemplateInput>) {
    const { sessionId, ...dto } = input;
  ...
  @Action({
    description:
      'Fetch chat history live from WhatsApp, bypassing the local DB. Useful for retrieving messages that arrived before the gateway started.',
    input: GetChatHistoryInput,
    kind: 'query',
    method: 'GET',
    path: 'sessions/:sessionId/messages/:chatId/history',
  })
  getChatHistory(input: z.infer<typeof GetChatHistoryInput>) {
    const includeMedia = input.includeMedia === true || input.includeMedia === 'true' || input.includeMedia === '1';
    return this.messageService.getChatHistory(input.sessionId, input.chatId, input.limit, includeMedia);
  }
